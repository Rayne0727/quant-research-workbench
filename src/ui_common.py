...
def render_home_page() -> None:
    ...
    st.subheader("如何使用")
    steps = (
        ("1. 上传数据", "选择明确的数据来源以及受支持的 CSV/XLSX 路径。"),
    ...
    st.markdown(
        ...
        "- **通用 CSV/XLSX**：读取文件并生成字段建议；用户确认映射、"
        "通过标准化预检和严格协议后，才可主动开始绩效分析。"
    )
    st.info(
        "系统不会自动确认字段、猜测收益率单位或修改原始数据。收益率使用小数格式，"
        "例如 0.01 表示 1%。"
    )
...
def render_help_page() -> None:
    ...
    with single_tab:
        ...
        st.markdown(
            "1. 选择示例数据，或选择现有标准协议上传 CSV。  \n"
            "2. 标准协议路径要明确选择标准日频收益或每周调仓净值格式。  \n"
            "3. 通用 CSV/XLSX 会先生成字段建议；请核对并确认字段映射，再生成标准化预览。  \n"
            "4. 标准化预检和严格协议均通过后，仍需最终确认并主动点击“开始绩效分析”。  \n"
            "5. 分析完成后查看指标、图表和摘要，再下载报告及标准化数据。"
        )
    ...
    with errors_tab:
        ...
        st.markdown(
            ...
            "- **CSV 编码无法识别**：另存为 UTF-8、UTF-8 BOM 或 GB18030。  \n"
            "- **XLSX 无法读取**：确认文件有效且未损坏，并明确选择工作表。  \n"
