...
当前版本：**v0.1.0-rc1 Release Candidate**。`rc1` 表示第一个发布候选版；当前计算、数据协议和导出口径保持冻结，通用文件路径支持确定性字段建议与用户确认式字段映射，但尚不执行标准化或绩效计算。

当前支持两条完整分析工作流，以及一条仅读取预览的导入路径：

1. 单实验分析：读取标准日频收益或每周调仓净值 CSV，验证、计算、绘图并导出标准化结果；
2. 多实验比较：读取 2 至 6 份标准化分析 CSV，按真实共同交易日期重新计算并比较。
3. 通用文件导入：读取 CSV 或 XLSX，确认解析设置、工作表和原始预览，给出确定性字段候选建议，再由用户选择策略主口径并显式确认字段映射；确认后仍不计算绩效。
...
建议和确认都不会修改、重命名或删除原始列，也不会自动判断百分数单位或换算数值。映射确认只保存字段引用，不会计算收益、净值、回撤或其他绩效，也不会生成分析报告或标准化下载；B.4 才会按照已确认映射执行标准化和严格数据验证。
