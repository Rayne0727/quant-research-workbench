...
当前版本：**v0.1.0-rc1 Release Candidate**。`rc1` 表示第一个发布候选版；当前计算、数据协议和导出口径保持冻结，本阶段只为通用文件预览增补确定性字段候选建议。

当前支持两条完整分析工作流，以及一条仅读取预览的导入路径：

1. 单实验分析：读取标准日频收益或每周调仓净值 CSV，验证、计算、绘图并导出标准化结果；
2. 多实验比较：读取 2 至 6 份标准化分析 CSV，按真实共同交易日期重新计算并比较。
3. 通用文件导入：读取 CSV 或 XLSX，确认解析设置、工作表和原始预览，并给出确定性字段候选建议，但不建立字段映射或计算绩效。
...
候选建议不会修改、重命名或删除原始列，也不会自动判断百分数单位或换算数值。即使显示“高置信度”，也只表示规则证据较强，不表示字段已经确认。当前不会依据建议计算收益、净值、回撤或其他绩效，也不会生成分析报告或标准化下载；用户确认字段映射属于 B.3 阶段。
