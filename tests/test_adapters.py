...
def test_valid_date_and_nav_can_be_processed() -> None:
    result = load_weekly_nav_csv(StringIO("date,nav_strat\n2026-01-01,1.0\n2026-01-02,1.1\n"))
...
def test_data_with_daily_return_can_be_processed() -> None:
    result = load_weekly_nav_csv(
        StringIO("date,nav_strat,daily_ret\n2026-01-01,1.0,0.0\n2026-01-02,1.1,0.1\n")
    )
...
def test_non_numeric_nav_fails() -> None:
    with pytest.raises(WeeklyNavValidationError, match=r"nav_strat.*无法转换"):
        load_weekly_nav_csv(StringIO("date,nav_strat\n2026-01-01,abc\n2026-01-02,1.1\n"))
...
def test_performance_cumulative_return_matches_final_normalized_nav() -> None:
    ...
    assert metrics["cumulative_return"] == pytest.approx(result.data["strategy_nav"].iloc[-1] - 1)


def test_non_numeric_daily_return_fails() -> None:
    with pytest.raises(WeeklyNavValidationError, match=r"daily_ret.*无法转换"):
        load_weekly_nav_csv(
            StringIO("date,nav_strat,daily_ret\n2026-01-01,1.0,abc\n2026-01-02,1.1,0.1\n")
        )
